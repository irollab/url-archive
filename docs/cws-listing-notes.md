...
- favicon：显示收藏站点的图标。
- optional tabs（运行时申请）：用户主动从新标签页剪藏最近浏览网页时，用于识别最近可剪藏标签页的 URL；默认不申请。
- optional host（http/https，运行时申请）：向用户自行配置的 Obsidian 与 LLM 端点发送数据；以及用户主动从新标签页剪藏最近浏览网页时访问该页面。默认不申请，仅在相关操作时按具体 origin 请求。
